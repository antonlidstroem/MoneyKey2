...
public class SickLeaveController : BaseApiController
{
    ...
    public async Task<IActionResult> GetAll(int budgetId)
    {
        ...
        // Users only see their own sick leave (privacy)
    ...
    public async Task<IActionResult> Create(int budgetId, [FromBody] CreateSickLeaveDto dto)
    {
        ...
        if (dto.EndDate < dto.StartDate) return BadRequest(new { Message = "Slutdatum måste vara efter startdatum." });
        var entry = new SickLeaveEntry { BudgetId = budgetId, UserId = UserId, StartDate = dto.StartDate,
            EndDate = dto.EndDate, SickLeaveType = dto.SickLeaveType, AnnualSgi = dto.AnnualSgi,
            GrossMonthlySalary = dto.GrossMonthlySalary, Notes = dto.Notes };
    ...
    {
        Id = e.Id, StartDate = e.StartDate, EndDate = e.EndDate, SickLeaveType = e.SickLeaveType,
        TotalDays = e.TotalDays, KarensDays = e.KarensDays, EmployerDays = e.EmployerDays, FkDays = e.FkDays,
        AnnualSgi = e.AnnualSgi, GrossMonthlySalary = e.GrossMonthlySalary,
        EmployerSickPay = e.EmployerSickPay, FkSickPay = e.FkSickPay,
        TotalBenefit = e.TotalBenefit, LostIncome = e.LostIncome,
        Notes = e.Notes, LinkedTransactionId = e.LinkedTransactionId
    };
}
